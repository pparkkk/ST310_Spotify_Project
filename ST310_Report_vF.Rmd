...
The original data set had 114,000 entries, 1000 for each of the 114 different values of track_genre. However, within this data set there were repeats of identical songs. This was due to various reasons such as having both the album version and single version, or being listed again with a different track_genre.  In addition, due to the large number of track_genres, there were many genres which were essentially the same, and also genres which weren't standard genres. For example, separate genres for metal and heavy-metal, or genres such as 'acoustic' or 'British' that included a wide overlap of other genres. 

In order to simplify the data, we created broader categories of genres and assigned each original track_genre into one or more of the wider genres. The wider genres we chose were Country, Electronic, Jazz, Pop, R&B, Rock, Classical, Metal, Other, World, Latin and Acoustic. This allowed us to use the track_genre data in our prediction without creating 113 dummy variables, as `tidymodels` has a limit of 58 categories when converting qualitative data into factors. In addition, it allowed us to list tracks under multiple genres for songs which had fusion genres, such as 'Rockabilly' which is a mixture of country and rock. This approach also allowed us to combine duplicate track entries without throwing away information on `track_genre`. 

In addition, when dealing with other duplicate track entries, we dropped tracks that had the same `track_name` and `artist`, but kept duplicates of `track_name`. This was because some entries were different covers of the same song, which often differed in other metrics as well as popularity. Our resulting data set had `track_id` as a unique identifier of entries with dummy variables representing all relevant genres. 

We did this data cleaning and formatting in python and then exported the resulting data sets as CSVs.Please find the python code used in the appendix. 
