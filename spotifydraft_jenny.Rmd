...
The original data set had 114,000 entries, 1000 for each of the 114 different values of track_genre. However, within this data set there were repeats of identical songs. This was due to various reasons such as having both the album version and single version, or being listed again with a different track_genre. 
In addition, due to the large number of track_genres, there were many genres which were essentially the same, and also genres which weren't standard genres. For example, separate genres for metal and heavy-metal, or genres such as 'acoustic' or 'British' that included a wide overlap of other genres. 
In order to simplify the data, we created broader categories of genres and assigned each original track_genre into one or more of the wider genres. The wider genres we chose were Country, Electronic, Jazz, Pop, R&B, Rock, Classical, Metal, Other, World, Latin and Acoustic. This allowed us to use the track_genre data in our prediction without creating 113 dummy variables, as TidyModels has a limit of 58 categories when converting qualitative data into factors. In addition, it allowed us to list tracks under multiple genres for songs which had fusion genres, such as 'Rockabilly' which is a mixture of country and rock. 
This approach also allowed us to combine duplicate track entries without throwing away information on track_genre. 
In addition, when dealing with other duplicate track entries, we dropped tracks that had the same track_name and artist, but kept duplicates of track_name. This was because some entries were different covers of the same song, which often differed in other metrics as well as popularity. 
We did this data cleaning and formatting in python and then exported the resulting data sets as CSVs.
Please find the python code used in the appendix. 
Our resulting data set had track_id as a unique identifier of entries with dummy variables representing all relevant genres. 
...
## [JENNY] Random forest 
For our high dimensional model we chose to implement random forests. Initially we wanted to tune all three hyperparameters, however this proved to be too long computationally to process. Instead  we tuned mtry as we thought this would have the most impact, and then separately tuned the other variables with the tuned mtry. Most values gave almost identical metrics, so we kept the initial values as they were the same to three decimal places.  
...
rf_recipe <- 
  recipe(popularity ~., data=data_train) %>%
  prep()
#Train mtry
rf_spec_1 <- 
  rand_forest(
...
rf_grid_1 <- grid_regular(finalize(mtry(),data_train),
                        levels = 5)
...
```
To avoid having to rerun the model we've exported one of the cross validation results as a csv.
We can see mtry=7 gives the best values for RMSE and rsq.

```{r}
#Export CV results for first random forest model 
write_csv(rf_res_1$.metrics[[1]], 'rf_1_metrics.csv', row.names = FALSE)
rf_1_metrics <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf_1_metrics.csv')
rf_1_metrics
```
...
```
Again, to avoid having to rerun the models, we exported the results to a csv. 
```{r}
# Predict on test data

write.csv(rf1_predict, "rf1_predict.csv", row.names = FALSE)
rf1_predict_results <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf1_predict.csv')

```

Below are the manual calculations of the random forest's predictions. 
```{r}
...
write.csv(rf_1_importance, 'rf_1_importance.csv', row.names = FALSE)
rf_1_importance <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf_1_importance.csv')
rf_imp_rownames <- rownames(rf_imp$importance)
rf_1_importance <- cbind(rf_1_importance, category=rf_imp_rownames)
rf_1_importance
```
...
rf_recipe2 <- 
  recipe(popularity ~., data=data_train2) %>%
...
As before, we export the metrics from cross validation. We can see mtry=17 has the best results, so this is what we shall use in the final model.
```{r}
write.csv(rf_res2$.metrics[[1]], 'rf_2_metrics.csv', row.names = FALSE)
rf_2_metrics <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf_2_metrics.csv')
rf_2_metrics
```
...
```{r}
rf2_predict
write.csv(rf2_predict, 'rf2_predict.csv', row.names=FALSE)
rf2_predict <- read_csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf2_predict.csv')
rf2_predict
```
...
adRSQ_rf2; RSQ_rf2
```
In order to gather the importance metrics like we did on the previous random forest, we first have to create dummy variables for the track_genre values. To do this, we've used dummy_cols from the fastDummies package. We then fitted the models with the same hyperparameters as above. 
```{r}
library(fastDummies)
...
rf_2_imp <- randomForest(data_train3[,-1:-2], data_train3$popularity, ntree = 300, mtry = 17, min_n = 50, importance = TRUE)
```

```{r}
rf_2_importance <- importance(rf_2_imp)
write.csv(rf_2_importance, 'rf_2_importance.csv', row.names = FALSE)
rf_2_importance <- read_csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf_2_importance.csv')
rf_2_imp_rownames <- rownames(rf_2_imp$importance)
rf_2_importance <- cbind(rf_2_importance, category=rf_2_imp_rownames)
rf_2_importance
```
