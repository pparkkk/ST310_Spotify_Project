...
```{r, echo = FALSE}
data_train <- read.csv("https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/training_data.csv",
                       header = TRUE)
data_test <- read.csv("https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/testing_data.csv",
                       header = TRUE)
```
...
**Comment**: The RMSE increases slightly compared to the training set, which is expected for a generalization. In addition, the R-squared decreases slightly to 2%. This implies that only 2% of the variation in popularity is being explained by the 3 predictors above. Although this is very poor, the model is very simple. So, we consider some extensions on this below.
...
```{r, echo = FALSE}
predictions_glmnet <- read.csv("https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/2.interpretable_glmnet.csv",
                       header = TRUE)
```
...
We illustrate the pseudo-Huber loss function for different values of $\delta$, with particular emphasis on the change of curvature from $L_1$ loss function to $L_2$.
...
``` {r, echo = FALSE}
y <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/y_gd.csv', header = TRUE)
y <- as.vector(y)$x
X <- as.matrix(read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/X_gd.csv', header = TRUE))
```
...
result <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/gd_result_delta_5.0.csv', header = TRUE)
...
``` {r, echo = FALSE}
gd_beta <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/gd_beta.csv', header = TRUE)
...
``` {r, echo = FALSE}
gd_pred <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/gd_pred.csv', header = TRUE)
...
Then, we manually calculate the performance metrics for comparability.
...
Tidymodels however in it's efforts to standardize the process meant that we were unable to retrieve the importance metrics associated with randomForest, so here we've rerun the model to gather the importance matrix, and then saved as a csv to avoid rerunning. 

The importance matrix returns 2 measures of "importance":
1. `%IncMSE` indicates the percentage the MSE would increase if a predictor is left out of the model, measured using the out-of-bag (OOB) samples.
2. `IncNodePurity` is the average reduction in training RSS after a split has been made on that predictor.

With regards to the output of the `randomForest` function, those variables that have higher values of both measures are deemed more important. `IncNodePurity`, however, can depends on the scale of the data, whereas `%IncMSE` is a standardized metric.
...
rf_1_importance <- rf_1_importance[order(rf_1_importance$X.IncMSE, decreasing = TRUE), ]

# Plot the results
ggplot(rf_1_importance, aes(x = Predictor, y = X.IncMSE)) +
  geom_bar(stat = "identity", color = "midnightblue", fill = "midnightblue") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 0.9, vjust = 0.2),
        panel.grid.major = element_blank()) +
  scale_x_discrete(limits = rf_1_importance$Predictor) +
  ggtitle("Importance Matrix Based on Percentage Increase in MSE") +
  labs(x = "Predictor", y = "%IncMSE")
```

``` {r, echo = FALSE}
rf_1_importance <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf_1_importance.csv', header = TRUE)
rf_imp_rownames <- colnames(data_train)[-1]
rf_1_importance <- cbind(Predictor = rf_imp_rownames, rf_1_importance)
rf_1_importance <- rf_1_importance[order(rf_1_importance$X.IncMSE, decreasing = TRUE), ]
...
```{r, echo = FALSE}
rf2_predict <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf2_predict.csv', 
                        header = TRUE)
```
...
# Order by importance based on %IncMSE
rf_2_importance <- rf_2_importance[order(rf_2_importance$X.IncMSE, decreasing = TRUE), ]

# Plot the results
ggplot(rf_2_importance[1:30,], 
       aes(x = Predictor, y = X.IncMSE)) +
  geom_bar(stat = "identity", color = "midnightblue", fill = "midnightblue") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 0.9, vjust = 0.2),
        panel.grid.major = element_blank()) +
  scale_x_discrete(limits = rf_2_importance$Predictor[1:30]) +
  ggtitle("Importance Matrix Based on Percentage Increase in MSE") +
  labs(x = "Predictor", y = "%IncMSE")
```

```{r, echo = FALSE}
rf_2_importance <- read.csv('https://raw.githubusercontent.com/pparkkk/ST310_Spotify_Project/main/rf_2_importance.csv', header = TRUE)
rf_2_imp_rownames <- colnames(data_train3)[-c(1,2)]
rf_2_importance <- cbind(Predictor = rf_2_imp_rownames, rf_2_importance)
rf_2_importance <- rf_2_importance[order(rf_2_importance$X.IncMSE, decreasing = TRUE), ]
