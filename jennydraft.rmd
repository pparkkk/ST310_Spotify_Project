...
data_raw <- read.csv("unique_tracks_genres.csv")
...
# Create new dummies showing if track has >1 genre
...
```{r}
# histogram plots for the non-binary 
col_names_non_binary <- c("popularity", "danceability", "energy", "key", "loudness", "speechiness", "acousticness", "instrumentalness", "liveness", "tempo", "time_signature")
par(mfrow = c(4, 3))
for (i in seq_along(col_names_non_binary)){
  hist(data[,col_names_non_binary[i]], main=col_names_non_binary[[i]])
}

# bar charts for the binary data
col_names_binary <- c("explicit", "mode", "pop", "rock", "country", "jazz", "electronic", "classical", "world", "kids", "other", "rap", "two_genre", "three_genre", "four_genre", "five_genre")
par(mfrow = c(4, 4)) 
for (i in seq_along(col_names_binary)){
  barplot(table(data[,col_names_binary[i]]), main=col_names_binary[[i]])
}

par(mfrow = c(1, 1)) # Reset the plotting area to the default (1 row and 1 column)
```
...
metrics <- c('popularity', 'danceability', 'energy', 'loudness', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence')
par(mfrow = c(4, 2)) 
for (i in metrics) {
  plots <- basic_plots(i)
...
These plots show the relation between the given metric (danceability, energy, speechiness, acousticness, instrumentalness, liveness, valence) against popularity. The colour coding on the right hand side indicates areas of high density in yellow. Adding to our previous suspicion, there does not appear to be any strong linear relations with these factors to popularity. If there had been, we would see a yellow line covering the x values.
...
```{r}
# Delete the two_genre dummy
data <- select(data, -two_genre, -three_genre, -four_genre, -five_genre)
data_test <- select(data_test, -two_genre, -three_genre, -four_genre, -five_genre)
...
The reason for choosing "danceability" is because it is one of the stongest (positively) correlated with popularity. Similarly for "instrumentalness", it is the strongest negatively correlated variable to popularity. "Explicit" was chosen as we thought it could be interesting to see how this (indicator) variable affects popularity (if at all). 
...
```{r}
sim_data <- data.frame(a = seq(-3, 3, length.out = 1000),
                       L1 = numeric(1000),
                       L2 = numeric(1000),
                       delta_0.5 = numeric(1000),
                       delta_1.5 = numeric(1000),
                       delta_2.0 = numeric(1000))

sim_data$L1 = abs(sim_data$a)
sim_data$L2 = sim_data$a^2
i <- 4
for (delta in c(0.5, 1.5, 2)) {
...
ggplot(sim_data) +
  ...
  geom_line(aes(a, delta_1.5, color = "1.5")) +
  geom_line(aes(a, delta_2.0, color = "2.0")) +
...
data$duration_ms <- log(data$duration_ms + 1)
data$loudness <- log(data$loudness - min(data$loudness) + 1)
data$speechiness <- log(data$speechiness + 1)
data$acousticness <- log(data$acousticness + 1)
data$instrumentalness <- log(data$instrumentalness + 1)
data$liveness <- log(data$liveness + 1)
...
**Comment**: This corresponds to a batch size of 1500, $\alpha$ = 0.6.
