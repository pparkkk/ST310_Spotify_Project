...
RSQ_baseline <- cor(data_valid$popularity, predictions_baseline)^2  # change this
...
y <- data$popularity
X <- as.matrix(data[,-c(1, 28)])  # ngenre is just a linear combination of other columns
...
fit <- lm(popularity ~ ., data = data[,-28])
...
**Comment**: The top 7 predictors, as measured by the magnitude of its corresponding slope coefficients, include 4 audio characteristics: `speechiness`, `instrumentalness`, `loudness` and `danceability`, and 3 genres: `classical`, `acoustic` and `r_n_b`. Afterwards, the magnitude of the coefficients are significantly lower. This agrees with the correlation plot since these variables seem to have higher correlation with popularity, despite being quite low in absolute value.
...
**Interpretation**: RMSE is now 18.27, slightly higher than the training RMSE, which is to be expected given it is a generalisation to another unseen data. The fact that RMSE hasn't increase much suggest that the algorithm didn't overfit the data. Similarly, adjusted R-squared saw a slight decline to about 11.9%. (compare to Ruby's)
